...
public class Path {
    ...
    public static double spacing = 2, SMOOTHING = 0.9, t = 0.001, k = 0.1;
    ...
    private double getPointVelocity(int p) {
        ...
        if(p <= 0) return Math.min(Math.sqrt(2 * MAX_ACCELERATION * d), k / getPointCurvature(p));

        return Math.min(Math.sqrt(Math.pow(path.get(p - 1).getVelocity(), 2) + 2 * MAX_ACCELERATION * d), Math.min(k / getPointCurvature(p), MAX_SPEED));
    }
    ...
    private double getPointNewVelocity(int p) {
        ...
        return Math.max(Math.min(path.get(p).getVelocity(), Math.min(Math.sqrt(Math.pow(path.get(p + 1).getVelocity(), 2) + 2 * MAX_ACCELERATION * d), MAX_SPEED)), MIN_SPEED);
    }
